    ...
    if os.path.exists(os.path.join(PROCESSED_DIR, arquivo_selecionado)):
        with st.spinner(f"Carregando dados de {mes_selecionado}..."):
            ...
            if df_mes is not None:
                ...
                # --- GRÁFICO TOP 10 VENDAS POR PDV E FILTRO DE TIPO DO TERMINAL ---
                st.subheader("Top 10 Vendas por PDV")
                tipo_top = st.selectbox("Tipo de Pagamento para o Top 10", ["LISTA", "PIX"], key="top_tipo")
                ...
                if df_top:
                    ...
                    df_top = df_top[df_top['TIPO DE PAGAMENTO'] == tipo_top]
                    if 'PDV' in df_top.columns and 'TIPO DO TERMINAL' in df_top.columns:
                        categorias = ['Todos'] + sorted(df_top['TIPO DO TERMINAL'].dropna().unique().tolist())
                        categoria_escolhida = st.selectbox("Categoria (TIPO DO TERMINAL)", categorias, key="top_categoria")
                        if categoria_escolhida != 'Todos':
                            df_top = df_top[df_top['TIPO DO TERMINAL'] == categoria_escolhida]
                        ...
                        fig_top = px.bar(
                            ...
                            y='PDV',
                            orientation='h',
                            title=f"Top 10 {'Menores' if ordem_top=='Menores' else 'Maiores'} Vendas em {tipo_top} por PDV",
                            color='VALOR',
                            color_continuous_scale='Blues' if tipo_top=="LISTA" else 'Greens',
                        )
                        ...
                        st.warning("Colunas 'PDV' ou 'TIPO DO TERMINAL' não encontradas nos arquivos.")
                else:
                ...
                if 'TIPO DE PAGAMENTO' in df_mes.columns and 'DATA/HORA' in df_mes.columns:
                    tipo_pag = st.selectbox("Selecione o tipo de pagamento para análise quinzenal", ['LISTA', 'PIX'], key="quinz_tipo")
                    df_quinz = df_mes[df_mes['TIPO DE PAGAMENTO'] == tipo_pag].copy()
                    ...
                    fig_quinz = px.bar(
                        ...
                        x='DATA',
                        y='VALOR',
                        title=f'Evolução Diária de Vendas em {tipo_pag} ({mes_selecionado})',
                        color_discrete_sequence=['#1f77b4'] if tipo_pag == 'LISTA' else ['#2ca02c']
                    )
                    ...
                    if not agrupado.empty:
                        ...
                        if primeira_quinzena > segunda_quinzena:
                            st.success(f"⬆️ **A 1ª quinzena teve mais vendas!** Diferença: R$ {(primeira_quinzena-segunda_quinzena):,.2f}")
                        elif segunda_quinzena > primeira_quinzena:
                            st.info(f"⬆️ **A 2ª quinzena teve mais vendas!** Diferença: R$ {(segunda_quinzena-primeira_quinzena):,.2f}")
                        else:
